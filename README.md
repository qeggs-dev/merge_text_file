# 文本文件合并工具 (Merge Text File)

这是一个用于合并文本文件的Python工具，特别适合将项目中的多个文本文件（如代码文件、文档等）合并成一个单一的输出文件。工具支持通过模板（Jinja2）自定义合并方式，并提供了灵活的文件包含和格式化选项。

## 功能特点

- 📁 **递归合并**：自动遍历目录，合并所有文本文件
- 🔧 **灵活配置**：支持通过JSON或YAML配置文件设置参数
- 🎨 **自定义包装**：提供多种文件内容包装方式（默认标记、Markdown代码块、自定义模板）
- 📝 **模板支持**：使用Jinja2模板语言，可动态生成合并结果
- 🎯 **文件过滤**：支持正则表达式模式匹配，只合并特定文件
- 📊 **日志记录**：详细的日志输出，便于调试和追踪
